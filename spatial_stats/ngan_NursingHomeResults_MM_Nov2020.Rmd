...
```{r }
nursing_home <- read.csv('../LTChomes_analysis_ON/data_output/merged_LTC_odhf_quality.csv')

# dput(names(nursing_home))
...
cols <- c( "first_inspection_date", 
           ...
           "date_max_HCWcases", 
           "date_first_resolved")
nursing_home[cols] <- lapply(nursing_home[cols], as.Date)
...
cols <- c("cleaned_name", 
          ...
          "postal_code", 
          "CSDname")
nursing_home[cols] <- lapply(nursing_home[cols], as.character)
...
# prepare data
nursing_home$logBed = log(nursing_home$number_beds)
...
ptCol <- colourScale(nursing_home$resident_deaths, breaks=colBreaks, style='fixed', col="YlOrRd")
...
pCols = mapmisc::colourScale(
  ...
  breaks=c(0.09735341, 0.09737589, 0.09738098, 0.09738324, 0.09738398, 0.09738524, 0.09742030), 
...
breaks = c(1.013618, 1.016363, 1.016476, 1.016533, 1.016576, 1.016767, 1.017030, 1.018372)
breaks = rev(breaks)
pCols = mapmisc::colourScale(
...
```{r Binomial Model}
Res7 = geostatsp::glgm(outbreak ~ number_beds
                                            # + total_complaints
                                            # + X2y_withOrders
                                            + home_type 
                                            # + total_inspections
...
pCols = mapmisc::colourScale(
  ...
  breaks = c(0.06451772, 0.15819085, 0.17997505, 0.19976422, 0.22515152, 0.29855273, 1.04889938),
    col='YlOrRd', rev=FALSE, style='fixed')
...
pCols = mapmisc::colourScale(
    ...
    breaks = c(0.3723564, 0.8965009, 1.0264569, 1.1213603, 1.2372540, 1.3896223, 1.8950715, 6.2884280),
    col='YlOrRd', rev=FALSE, style='fixed')
