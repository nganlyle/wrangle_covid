...
nursing_home$Total.Confirmed.Cases <- as.numeric(as.character(nursing_home$Total.Confirmed.Cases))
...
```{r, echo = F}
nursing_home
ptCol2 <- colorFactor("YlOrRd", nursing_home$anyCases, na.color = "#000000")
legendLabs2 <- unique(nursing_home$anyCases)

mapcolours <- unique(ptCol2(nursing_home$anyCases))

leaflet(nursing_home) %>%
  addTiles() %>%
  addCircleMarkers(#lng = ~longitude, lat = ~latitude, 
    label = ~ anyCases, 
             fillColor = ptCol2(nursing_home$anyCases), fillOpacity = 0.8, radius = 4, stroke = FALSE) %>%
  addLegendCustom(colors = mapcolours, labels = legendLabs2, sizes = 6, font.sizes = 2, group='Outbreak Status')
