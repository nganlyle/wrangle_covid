...
```{r}
## REMOVING PREDICTORS UNRELATED TO OUTBREAK - reverse commented code for predictors to deaths
profile <- c("DEPRESSION", "ADL_DEPENDENT", "CPS_SEVERE", "DEMENTIA", "ABS_PHYS_ABUSIVE",
             "MEDS_DEPRESSION", "MEDS_ANTIPSYCHOTICS", "STAY_LENGTH")
df3 <- df3[, -which(names(df3) %in% profile)]
...
```
  ...
  * Bin `ISE_LOW`, `SUB_COMPLAINTS`, `CMI` to replace na's with "unknown" **will convert to character but preserve NAs**
...
# binary logistic on deaths

```{r}
# df_deaths <- df3[,-c(1,23)]
# 
# df_deaths <- subset(df_deaths, df_deaths$deaths != "no outbreak")
# 
# fit3 = glm(deaths~., family=binomial, data=df_deaths, maxit = 25)
# summary(fit3)
