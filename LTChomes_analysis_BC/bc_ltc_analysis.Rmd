...
# Import the data
df <- read.csv("../data/bc_ltc_final.csv", na.strings="", header=TRUE)
```
...
rmv <- c("FACILITY_NAME", "HCC_CODE", "HLTH_AUTH", "STREET_ADDRESS", "CITY", "POSTAL", "PHONE", "OPEN_DATE", 
         "COUNCIL", "ACCRED_STATUS", "INCIDENT_OUTBREAK", "INCIDENT_ABUSE", "INCIDENT_FALL", 
         "INCIDENT_POISON", "INCIDENT_MEDICATION", "INCIDENT_WANDERING", "INCIDENT_INJURY", "INCIDENT_AGGRESSION",
        'latitude', 'longitude', 'point',
        'Number.of.dead', # removing number of dead because we are using outbreak as a response
        'ownership_type') # removing ownership type because it seems to have perfect multicollinearity with home_type
...
**Assessing sparsity of dataframe:**
...
par(mfrow=c(2,2))
hist(xtabs(~ROOMS_SEMI_PROP + BEDS_TOTAL, data=df4), main = "INCIDENT_FALL_100 Untransformed")
hist(xtabs(~ROOMS_SEMI_PROP + BEDS_TOTAL, data=df5), main = "INCIDENT_FALL_100 LOG Transformed")

# sqrt transform
hist(xtabs(~sqrt(ROOMS_SEMI_PROP) + BEDS_TOTAL, data=df4), main = "INCIDENT_FALL_100 SQRT Transformed")
...
# which facilities have high amounts of NAs?
df[c(28, 32, 47, 211),]$FACILITY_NAME
```

NAs are often concentrated for particular homes and in incident predictors.

```{r}
df[is.na(df4$years_operating),][1:6]
```
...
----------|--------------|-------------
Linearity - linear relationship between continuous predictor variables and the logit of the outcome | visually - scatterplot | many appear non-linear
Influential values | Cook's distance plot of model fit | no obvious influential points
Multicolinearity | `car::vif(model)` | potentially perfect multicollinearity between `ownership_type` and `home_type` **RESOLVED:** 

```{r linearity check, suppressMessages = TRUE}
library(tidyverse)
library(broom)
...
Between 1 and 11 predictors are necessary.
