...
all_inspections = c(complaints, noncomplaints, critical, withOrders)
all_inspections_general = c(complaints, noncomplaints)
all_inspections_specific = c(complaints, critical, withOrders)
...
# 
# recent_inspections = c(complaints_recent,  noncomplaints_recent)
# recent_inspections_specific = c(complaints_recent, critical_recent, withOrders_recent)

```

## Analysis with all 625 homes (exclude Quality data)
```{r }
data <- subset(df, select = c('outbreak', profile, all_inspections))
skim(data)
# View(colnames(data))
# dput(names(df))
```
...
As lambda decreases, the order that covariates are added to the model:
    ...
    7. family_councilYes, 
    8. total_noncomplaints_norm
    9. X5y_noncomplaints_norm
    10. accreditationYes
    11. X2y_noncomplaints_norm
