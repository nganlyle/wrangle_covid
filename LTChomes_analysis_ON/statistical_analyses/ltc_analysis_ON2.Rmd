...
data <- subset(df, select = c(keep, profile, all_inspections, noncomplaints, complaints))
skim(data)
# dput(names(df))
```
...
outbreak <- x[,1]
home_type <- as.factor(x[,2])
short_stay <- as.factor(x[,4])
residents_council <- as.factor(x[,5])
family_council <- as.factor(x[,6])
accreditation <- as.factor(x[,7])


xfactors <- model.matrix(outbreak~home_type+short_stay+residents_council+family_council+accreditation)[,-1]

x <- as.matrix(data.frame(x[,-c(1, 2, 4, 5, 6, 7)], xfactors))

# Note alpha=1 for lasso only and can blend with ridge penalty down to
# alpha=0 ridge only.
glmmod <- glmnet(x, y=as.factor(outbreak), alpha=1, family="binomial")
...
cv.glmmod <- cv.glmnet(x, y=outbreak, alpha=1)
plot(cv.glmmod)
