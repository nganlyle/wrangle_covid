...
inspections_total = c(
  ...
  'total_withOrders_per_year'
)

inspections_5y = c(
    'X5y_inspections_per_year'
  , "X5y_withOrders_per_year"
  )

inspections_2y = c(
    'X2y_inspections_per_year'
  , "X2y_withOrders_per_year"
  )
...
data <- subset(df_southON, select = c('outbreak', profile, inspections_5y, inspections_2y))
# skim(data)
# View(colnames(data))
# dput(names(df))
```
...
## Fit logistic regression - selected covariates
```{r }
fit = glm(outbreak~
            number_beds
          # + home_type
