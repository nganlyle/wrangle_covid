...
df <- df[,-c(1:2, 4:5, 7:8, 10:32, 43:52, 57:59, 63, 78:79)] 
...
## PCA on the data
```{r }
pca <- prcomp(df[,-c(1, 35)], scale = TRUE)
summary(pca)
...
pcr_data <- as.data.frame(cbind(df[,"count_prop"], pcs))
head(pcr_data)
...
# # Backwards Selection
# phu_lm_total <- update(phu_lm_total, .~. -PC5)
# summary(phu_lm_total)
# phu_lm_total <- update(phu_lm_total, .~. -PC3)
# summary(phu_lm_total)
# phu_lm_total <- update(phu_lm_total, .~. -PC4)
# summary(phu_lm_total)
# phu_lm_total <- update(phu_lm_total, .~. -PC6)
# summary(phu_lm_total)
# phu_lm_total <- update(phu_lm_total, .~. -PC2)
# summary(phu_lm_total)
